# -*- coding: utf-8 -*-
import writereadFile,rulesGet,reBetaPattern
import json,pickle
import numpy as np
...
if __name__=='__main__':
    ...
    for attr_i in range(len(attrlist)):
        ...
        for i in clusDict1.keys():
            ...
            templateDicts1[i] = reBetaPattern.reTemplate(clusDict1[i][:int(0.8*len(clusDict1[i]))])
            templateDicts2[i] = {}
            clus2_i = clusDict2[i]
            ...
            for j in clus2_i.keys():
                if clus2_i[j] == clusDict1[i]:
                    ...
                    templateDicts2[i][j] = reBetaPattern.reTemplate(clus2_i[j][:int(0.8*len(clus2_i[j]))])
        ...
        alltestsent.extend(alltestsent)
        reSentmatchListRoot,reSentmatchDict1,reSentmatchDict2 = \
            reBetaPattern.reSentsMarch(tests_sent,templateDictsRoot,templateDicts1, templateDicts2)
    ...
    for sent in alltestsent:
        sent = reBetaPattern.delNum(sent)
        ks = 0
        ko = 0
        da = sent.split(' ')
        for word in da:
        ...
        marchvalue0, marchIndex0 = reBetaPattern.reModel(allrelBoD, cutSen, indexsWord)
        if marchvalue0 > 0:
            if marchIndex0 not in sentmarch_all.keys():
                sentmarch_all[marchIndex0] = [sent]
            else:
                sentmarch_all[marchIndex0].append(sent)
    ...
    print('end')
    write1 = open('plotAll/' + url1 + '_all_beta' + '.pkl', 'wb')
